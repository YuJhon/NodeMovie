 ...
 * @author jiangyu
 * @create 2017-02-03 22:56
 * @version v1.0
 */
(function (app) {
    'use strict';
    //app.controller()方法的第一个参数是controller名称，第二个参数为数组，数组的前面是声明注入的内容，可以是n个，最后一个是个function，function的参数个数也必须是n个，必须跟前面声明注入的内容一一对应
    app.controller('AddController',['$scope', '$location','Upload', '$state','MovieService', 'DIR', function ($scope, $location,Upload, $state, MovieService, DIR) {
        document.title = '电影网站';
        //初始化数据库存的图片名称
        $scope.serverPicName = '';
